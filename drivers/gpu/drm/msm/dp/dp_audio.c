...
static void msm_dp_audio_setup_sdp(struct msm_dp_audio_private *audio,
				   enum msm_dp_stream_id stream_id)
{
	/* The shipping driver always programs stream 0 before stream 1. */
	msm_dp_audio_config_sdp_stream(audio, MSM_DP_STREAM_0);
	if (stream_id == MSM_DP_STREAM_1)
		msm_dp_audio_config_sdp_stream(audio, MSM_DP_STREAM_1);
