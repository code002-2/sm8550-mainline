				     ...
				     enum usb_role next_role)
{
	...
	if (qcom->connector_role != next_role) {
		...
		/* A connector role change supersedes a deferred host restart. */
				   ...
				   struct typec_mux_state *state)
{
	...
	if (svid == USB_TYPEC_DP_SID) {
		...
		dp_only = false;
		/* Physical detach must not restart host before DP releases the PHY. */
		defer_host_restart = true;
	} else {
		return 0;
	}
